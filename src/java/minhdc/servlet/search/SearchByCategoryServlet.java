...
public class SearchByCategoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String category = request.getParameter("categorySearchValue");
        ...
        if (user != null) {
            ...
            try {
                ...
                arrListItem = dao.loadCategorySearchResult(category,isAdmin);
                Collections.sort(arrListItem);
                session.setAttribute("ITEMSLIST", arrListItem);
                request.setAttribute("categorySearchValue", category);
                request.setAttribute("ACTION", "searchByCategory");

            } catch (SQLException ex) {
                log("SearchByCategoryServlet_SQL " + ex.getMessage());
            } catch (NamingException ex) {
                log("SearchByCategoryServlet_Naming " + ex.getMessage());
            } finally {
            ...
            try {
                ...
                Collections.sort(arrListItem);
                if (arrListItem == null) 
                    request.setAttribute("ERROR", "This category is run out product!");
                session.setAttribute("ITEMSLIST", arrListItem);
                request.setAttribute("categorySearchValue", category);
                request.setAttribute("ACTION", "searchByCategory");
