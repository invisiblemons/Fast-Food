 ...
 * @author MONS
 */
@WebServlet(name = "SearchNameOrderServlet", urlPatterns = {"/SearchNameOrderServlet"})
public class SearchNameOrderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String name = request.getParameter("searchName_Order");
        ...
        try {
            ...
            if (null == name || "".equals(name.trim())) {
                ...
                for (Map.Entry<Integer, ArrayList<TblOrderDetailDTO>> entry : listDetail.entrySet()) {
                    for (TblOrderDetailDTO tblOrderDetailDTO : entry.getValue()) {
                        if (tblOrderDetailDTO.getProductName().toLowerCase().contains(name.toLowerCase())) {
                            if (!orderListSearch.contains(entry.getKey())) {
                                orderListSearch.add(entry.getKey());
                            }
                        }
                    }
                }
            ...
            log("SearchNameOrderServlet_SQL " + ex.getMessage());
        } catch (NamingException ex) {
            log("SearchNameOrderServlet_Naming " + ex.getMessage());
